...
const rootRoute = createRootRoute({
    component: () => (_jsx(AppShell, { children: _jsx(Outlet, {}) })),
});
const route = (path, component, beforeLoad) => beforeLoad
    ? createRoute({ getParentRoute: () => rootRoute, path, component: component, beforeLoad })
...
const relationshipsRoute = route('/relationships', P.RelationshipsPage, requirePermission(PERMISSIONS.RELATIONSHIPS_READ));
const contractsRoute = route('/contracts', P.ContractsPage, requirePermission(PERMISSIONS.CONTRACTS_READ));
...
const consentRoute = route('/consent', P.ConsentPage, requirePermission(PERMISSIONS.CONSENT_READ));
...
export const router = createRouter({
    routeTree,
    defaultErrorComponent: ({ error, reset }) => _jsx(RouteError, { error: error, reset: reset }),
    defaultNotFoundComponent: () => _jsx(NotFound, {}),
});
export function AppRouter() {
    return (_jsx(AppErrorBoundary, { children: _jsx(RouterProvider, { router: router }) }));
}
