...
export type CashCategory =
  | 'premium' | 'claim' | 'commission' | 'payroll' | 'supplier'
  | 'rent' | 'software' | 'tax' | 'loan' | 'refund' | 'fee' | 'reinsurance';

export type Account = 'HSBC' | 'BK' | 'I&M' | 'MTN MoMo' | 'Airtel';

export interface LinkedRecord {
  kind: 'invoice' | 'claim' | 'bill' | 'contract' | 'payroll' | 'policy';
  ref: string;
}
...
export interface CashMovement {
  ...
  purpose: string; // plain-English "why"
  account: Account;
  direction: Direction;
...
export const CATEGORY_META: Record<CashCategory, { label: string; tone: string; defaultDir: Direction }> = {
...
export const ACCOUNTS: Account[] = ['HSBC', 'BK', 'I&M', 'MTN MoMo', 'Airtel'];
