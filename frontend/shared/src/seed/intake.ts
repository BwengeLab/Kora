...
export type IntakeStage = 'extracting' | 'needs-review' | 'matched' | 'posted';
export type IntakeSource = 'bank-feed' | 'email' | 'scan' | 'upload';
export type IntakeKind = 'invoice' | 'receipt' | 'statement' | 'contract';
...
export interface IntakeDoc {
  ...
  suggestedMatch?: { ref: string; party: string; amount: string } | undefined;
}

export const SOURCE_META: Record<IntakeSource, { label: string }> = {
...
const f = (label: string, value: string, confidence: number): ExtractedField => ({ label, value, confidence });

export const seedIntake: IntakeDoc[] = [
  {
