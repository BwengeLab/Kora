...
export interface RoiItem {
  ...
  label: string;
  value: Money;
  icon: 'recovered' | 'duplicates' | 'unsupported' | 'hours' | 'leakage' | 'credit';
  detail: string;
  deltaPct: number;
}
...
export const seedRoi = {
  ...
  roiMultiple: 8.0,
  series: [42, 58, 71, 96, 128, 161] as number[],
...
// ── Reports ─────────────────────────────────────────────────────────────────
export type ReportKind = 'executive' | 'board' | 'exception' | 'collections' | 'supplier' | 'credit' | 'audit';
...
export const seedOverdue: Overdue[] = [
