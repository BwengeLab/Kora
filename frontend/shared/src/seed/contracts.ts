...
const M = (amount: number, currency = 'USD'): Money => ({ amountMinor: BigInt(Math.round(amount * 100)), currency });

export type ContractType = 'policy' | 'vendor' | 'lease' | 'partner' | 'reinsurance' | 'service';
export type ContractStatus = 'active' | 'renewal-due' | 'expiring' | 'expired' | 'draft';
...
export const CONTRACT_TYPE_META: Record<ContractType, { label: string; tone: string }> = {
