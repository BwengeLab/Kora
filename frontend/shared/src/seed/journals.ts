...
export type JournalSource = 'opening' | 'manual' | 'AP' | 'AR' | 'bank' | 'payroll' | 'tax' | 'claims';
export type JournalStatus = 'posted' | 'draft';

export interface JournalLine {
  account: string; // CoA code
  debit: bigint; // minor units
  credit: bigint; // minor units
  costCenter?: string;
}
...
const d = (n: number): bigint => BigInt(Math.round(n * 100));
const dr = (account: string, amount: number, costCenter?: string): JournalLine => ({ account, debit: d(amount), credit: 0n, ...(costCenter ? { costCenter } : {}) });
const cr = (account: string, amount: number, costCenter?: string): JournalLine => ({ account, debit: 0n, credit: d(amount), ...(costCenter ? { costCenter } : {}) });
