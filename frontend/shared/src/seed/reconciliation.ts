...
// Where an item sits in its lifecycle. Operator owns up to `prepared`.
export type ReconStage = 'detected' | 'reviewing' | 'prepared' | 'approved' | 'posted';
...
export interface EvidenceDoc {
  id: string;
  name: string;
  kind: 'statement' | 'invoice' | 'receipt' | 'contract' | 'po';
  sizeText: string;
  pageRef?: string; // e.g. "p.3, line 14"
}
...
const agentDetected = (at: string): HistoryEvent => ({
  id: `h-${at}`,
  at,
...
export const seedReconciliations: Reconciliation[] = [
  {
    ...
    tier: 'suggested',
    reason:
      'The amount and party match exactly, the reference is 87% similar (ACME-INV-10356 ↔ INV-10356), and the dates are one day apart — a normal settlement gap.',
    ageText: '2h ago',
    deltas: [
  ...
  },
  {
    ...
    tier: 'suggested',
    reason:
      'The amount matches a recurring monthly rent. References differ but the 2-week gap to the bill date is normal for this lease, and the party matches.',
    ageText: '4h ago',
    deltas: [
  ...
  },
  {
    ...
    tier: 'review',
    reason:
      'The amount is $260 higher than the purchase order and there is no reference on the bank side. The party is similar but this needs a human to confirm the overcharge or a partial delivery.',
    ageText: '1h ago',
    deltas: [
  ...
  },
  {
    ...
    tier: 'suspicious',
    reason:
      'Unknown counterparty with no contract or PO on file, large round amount, posted outside business hours. Recommend escalation to the Audit agent.',
    ageText: '3d ago',
    deltas: [],
...
// Per-tier counts + money value for the clickable tier filters.
...
export const seedTierStats: TierStat[] = [
