...
// The agent activity engine. Running an agent performs REAL work against the live
// stores (or reads them) and logs observable events to a feed — so you can watch
// agents do their jobs and see the changes propagate (e.g. the Reconciliation
// Agent moves detected items into review, visible on the reconciliation pages).

export type EventTone = 'info' | 'success' | 'warning' | 'danger' | 'ai';
...
interface AgentActivityState {
  ...
  processed: Record<string, number>; // extra items processed this session
...
let seq = 0;
const ev = (agentId: string, agentName: string, action: string, detail: string, tone: EventTone, link?: AgentEvent['link']): AgentEvent => ({
  id: `ae-${Date.now()}-${seq++}`,
  agentId,
  agentName,
  at: Date.now(),
...
const RUNNERS: Record<string, () => { events: AgentEvent[]; processed: number }> = {
  'a-recon': () => {
...
const genericRun = (id: string): { events: AgentEvent[]; processed: number } => ({
