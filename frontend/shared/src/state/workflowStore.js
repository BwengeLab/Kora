...
import { seedAuditLog } from '../seed/auditorHome';
import { seedReconciliations, } from '../seed/reconciliation';
...
function riskFromTier(tier) {
    if (tier === 'suspicious' || tier === 'review')
        return 'high';
    if (tier === 'duplicate')
        return 'medium';
    return 'low';
...
const isOwnerRole = (role) => /owner/i.test(role);
...
export function approvalBlockReason(item, actorRole, actorName) {
    if (item.stage === 'approved' || item.stage === 'rejected')
        return null;
    if (item.isOwnItem)
        return 'sod';
    if (item.approvals.some((a) => a.name === actorName))
        return 'duplicate';
    if (item.requiresDualApproval && item.approvals.length === 0 && isOwnerRole(actorRole))
        return 'needs-first';
    return null; // null here = no block → may approve
}
// Deep-ish clones so we never mutate the seed module arrays.
const cloneRecons = () => seedReconciliations.map((r) => ({ ...r, history: [...r.history] }));
const cloneApprovals = () => seedApprovals.map((a) => ({ ...a, approvals: [...a.approvals], history: [...a.history] }));
const cloneAudit = () => seedAuditLog.map((e) => ({ ...e }));
function auditEvent(item, by, action, kind) {
    return {
...
export const useWorkflowStore = create((set, get) => ({
    ...
    prepareMatch: (reconId, by) => {
        ...
        if (!recon || recon.stage === 'prepared')
            return;
        // 1. advance the reconciliation
        set((s) => ({
            reconciliations: s.reconciliations.map((r) => r.id === reconId
                ? {
                    ...r,
                    stage: 'prepared',
        ...
        const limit = { amountMinor: BigInt(Math.round(dualThreshold() * 100)), currency: recon.transaction.amount.currency };
        const approvalId = `ap-from-${reconId}`;
        if (get().approvals.some((a) => a.id === approvalId))
            return;
        const newApproval = {
            id: approvalId,
            type: 'match',
    ...
    agentSuggestMatches: (max = 2) => {
        ...
        set((s) => ({
            reconciliations: s.reconciliations.map((r) => ids.has(r.id)
    ...
    rejectReconciliation: (reconId) => {
        set((s) => ({
            reconciliations: s.reconciliations.map((r) => r.id === reconId ? { ...r, stage: 'detected', ageText: 'Rejected — back to review' } : r),
        }));
    },
    ...
    approve: (approvalId, by) => {
        ...
        const nextStage = needsSecond ? 'partial' : 'approved';
        ...
        set((s) => ({
            ...
            reconciliations: s.reconciliations.map((r) => `ap-from-${r.id}` === approvalId && !needsSecond ? { ...r, stage: 'posted' } : r),
