...
import { accountByCode, seedChartOfAccounts } from '../seed/chartOfAccounts';
...
export const linesBalanced = (lines) => {
    const dr = lines.reduce((a, l) => a + l.debit, 0n);
    const cr = lines.reduce((a, l) => a + l.credit, 0n);
    return dr === cr && dr > 0n;
};
export const useGLStore = create((set) => ({
    journals: seedJournals,
    hydrate: (journals) => set({ journals }),
    postJournal: (entry) => {
        if (!linesBalanced(entry.lines))
            return false;
        const full = { ...entry, id: `je-${Date.now()}`, status: 'posted' };
...
const inScope = (e, scope) => scope === 'all' || e.entity === scope;
// Net balance per account (debit-positive), posted entries only, scoped.
export function accountNets(journals, scope) {
    const m = new Map();
    for (const e of journals) {
        if (e.status !== 'posted' || !inScope(e, scope))
            continue;
        for (const l of e.lines)
            m.set(l.account, (m.get(l.account) ?? 0n) + l.debit - l.credit);
    }
    return m;
}
// Trial balance — each account's net shown in its natural column; totals tie out.
export function trialBalance(journals, scope) {
...
export function displayBalance(code, journals, scope) {
...
export function incomeStatement(journals, scope) {
    const nets = accountNets(journals, scope);
    const lineFor = (type) => seedChartOfAccounts
...
export function balanceSheet(journals, scope) {
    const nets = accountNets(journals, scope);
    const section = (type) => seedChartOfAccounts
        .filter((a) => a.type === type)
        .map((a) => ({ code: a.code, name: a.name, amount: (a.normal === 'credit' ? -1n : 1n) * (nets.get(a.code) ?? 0n) }))
        .filter((l) => l.amount !== 0n);
...
export function cashFlow(journals, scope) {
    let opening = 0n;
    let operating = 0n;
    let investing = 0n;
    let financing = 0n;
    for (const e of journals) {
        if (e.status !== 'posted' || (scope !== 'all' && e.entity !== scope))
            continue;
