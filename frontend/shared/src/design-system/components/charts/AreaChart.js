...
export function AreaChart({ xLabels, series, height = 220, showLegend = false, guideOnHover = true }) {
    ...
    const option = useMemo(() => ({
        ...
        yAxis: {
            ...
            axisLabel: {
                ...
                fontSize: 11,
                formatter: (v) => (Math.abs(v) >= 1e6 ? `$${(v / 1e6).toFixed(0)}M` : `$${(v / 1e3).toFixed(0)}K`),
            },
        },
        series: series.map((s) => ({
            name: s.name,
    ...
    return (_jsx("div", { ref: containerRef, style: { height, width: '100%' }, children: _jsx(ReactECharts, { ref: chartRef, option: option, style: { height: '100%', width: '100%' }, opts: { renderer: 'svg' }, notMerge: true }) }));
}
function hexA(hex, a) {
    // #rrggbb → rgba(r,g,b,a)
    const h = hex.replace('#', '');
