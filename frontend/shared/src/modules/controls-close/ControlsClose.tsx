...
import { seedCloseTasks, seedControlChecks, seedEvidenceGaps, type CloseTask, type ControlCheck, type EvidenceGap } from '../../seed/financeLeadClose';
...
export function ControlsClose() {
  ...
  const [evidenceGaps, setEvidenceGaps] = useState<EvidenceGap[]>(seedEvidenceGaps);
  const [controlChecks, setControlChecks] = useState<ControlCheck[]>(seedControlChecks);
  ...
  const requestGap = async (gap: EvidenceGap) => {
    if (token) {
        ...
        <section className="grid grid-cols-1 gap-4 @3xl:grid-cols-[auto_1fr]">
          <GlassSurface tone="strong" className="flex items-center gap-5 p-5">
            ...
            <div>
              ...
              <button type="button" disabled={!allDone} onClick={() => void lockPeriod()} className={cn('mt-2 inline-flex h-9 items-center gap-1.5 rounded-xl px-3.5 text-[12px] font-bold', allDone ? 'bg-gradient-to-br from-brand to-brand-ink text-white shadow-glass-soft hover:brightness-110' : 'cursor-not-allowed bg-white/50 text-ink-muted ring-1 ring-white/60')}><Lock className="size-3.5" /> Lock the period</button>
            </div>
          </GlassSurface>
          <div className="grid grid-cols-3 gap-3">
            <MiniMetric label="Exceptions to clear" value="2" tone="text-danger" icon={<AlertTriangle className="size-4" />} />
            <MiniMetric label="Evidence gaps" value={String(evidenceGaps.length)} tone="text-warning" icon={<FileWarning className="size-4" />} />
              ...
              {grouped.map(([area, items]) => (
                <div key={area}>
                  ...
                  <ul className="flex flex-col gap-1.5">
                    {items.map((task) => (
                      <li key={task.id}>
                        <button type="button" onClick={() => void toggle(task.id)} className={cn('flex w-full items-center gap-3 rounded-2xl p-3 text-left ring-1 transition-colors', task.done ? 'bg-success-soft/40 ring-success/15' : task.blocked ? 'bg-danger-soft/30 ring-danger/15' : 'bg-white/55 ring-white/60 hover:bg-white')}>
            ...
            <GlassSurface tone="strong" className="flex flex-col gap-2.5 p-5">
              ...
              {evidenceGaps.map((gap) => (
                <div key={gap.id} className="flex items-center gap-2 rounded-xl bg-white/55 p-2.5 ring-1 ring-white/60">
                  ...
                  <button type="button" onClick={() => void requestGap(gap)} className="shrink-0 rounded-lg bg-brand px-2 py-1 text-[10.5px] font-bold text-white hover:brightness-110">Request</button>
                </div>
              ))}
            </GlassSurface>
