...
// Delegation-of-Authority matrix editor — the configurable approval rulebook.
export function DoaMatrix() {
  const rules = useApprovalPolicyStore((s) => s.rules);
  const addRule = useApprovalPolicyStore((s) => s.addRule);
  const updateRule = useApprovalPolicyStore((s) => s.updateRule);
  const removeRule = useApprovalPolicyStore((s) => s.removeRule);
  const [editing, setEditing] = useState<ApprovalRule | null>(null);
  const [isNew, setIsNew] = useState(false);

  const openNew = () => { setEditing(blankRule()); setIsNew(true); };
  const openEdit = (r: ApprovalRule) => { setEditing(r); setIsNew(false); };
  const save = (r: ApprovalRule) => {
    if (isNew) addRule(r); else updateRule(r.id, r);
    setEditing(null);
    toast({ tone: 'success', title: isNew ? 'Rule added' : 'Rule saved', body: `“${r.label}” is now active in the approval matrix.` });
  };
    ...
    <div className="flex flex-col gap-5">
      ...
      >
        <div className="overflow-hidden rounded-2xl ring-1 ring-white/60">
          ...
          <ul>
            {rules.map((r) => (
              <li key={r.id} className="grid grid-cols-[1.4fr_1fr_1fr_1.6fr_auto] items-center gap-3 border-t border-white/45 bg-white/30 px-4 py-3">
                <div className="min-w-0"><p className="truncate text-[13px] font-bold text-ink">{r.label}</p><p className="text-[10.5px] text-ink-muted">{r.requireEvidence ? 'Evidence required' : 'No evidence rule'}</p></div>
                <span className="text-[12px] text-ink-soft">{r.category === 'all' ? 'All types' : cap(r.category)} · {r.scope === 'all' ? 'All entities' : entityName(r.scope)}</span>
                <span className="text-[12.5px] font-semibold tabular text-ink">{fmtBand(r)}</span>
                <div className="flex items-center gap-1">
                  {r.approvers.map((a, i) => (
                    <span key={a} className="inline-flex items-center gap-1">
                      <span className={cn('rounded-full px-2 py-0.5 text-[10px] font-bold', i === 0 ? 'bg-brand-soft text-brand-ink' : 'bg-lavender-soft text-lavender')}>{shortRole(a)}</span>
                      {i < r.approvers.length - 1 ? <ArrowRight className="size-3 text-ink-muted" /> : null}
                    </span>
                  ))}
                  {r.approvers.length >= 2 ? <span className="ml-1 rounded-full bg-warning-soft px-1.5 py-0.5 text-[9px] font-bold uppercase text-warning">dual</span> : null}
                </div>
                <div className="flex items-center gap-1">
                  <button type="button" onClick={() => openEdit(r)} className="grid size-8 place-items-center rounded-lg text-ink-muted hover:bg-white/70 hover:text-ink" title="Edit"><Pencil className="size-3.5" /></button>
                  <button type="button" onClick={() => { removeRule(r.id); toast({ tone: 'warning', title: 'Rule removed', body: `“${r.label}” deleted from the matrix.` }); }} className="grid size-8 place-items-center rounded-lg text-ink-muted hover:bg-danger-soft hover:text-danger" title="Delete"><Trash2 className="size-3.5" /></button>
                </div>
              </li>
            ))}
          </ul>
...
// Try-it: enter an amount/category/entity and see the resolved chain live.
function PolicySimulator() {
  ...
  const result = resolveChainFrom(rules, parseFloat(amount || '0'), { category, scope });
  ...
  return (
    <SettingsCard title="Test the policy" desc="Enter a transaction and see exactly who would need to approve it.">
      <div className="grid grid-cols-1 gap-4 @2xl:grid-cols-[1fr_1fr_1fr_1.4fr]">
        <Field label="Amount (USD)"><input value={amount} onChange={(e) => setAmount(e.target.value.replace(/[^0-9.]/g, ''))} inputMode="decimal" className={inputCls} /></Field>
        <Field label="Category"><select value={category} onChange={(e) => setCategory(e.target.value as RuleCategory)} className={inputCls}>{RULE_CATEGORIES.map((c) => <option key={c} value={c}>{c === 'all' ? 'All types' : cap(c)}</option>)}</select></Field>
        <Field label="Entity"><select value={scope} onChange={(e) => setScope(e.target.value as EntityScope)} className={inputCls}><option value="all">All entities</option>{seedEntities.map((e) => <option key={e.id} value={e.id}>{e.name}</option>)}</select></Field>
        <div className="flex flex-col gap-1">
          ...
          <div className="flex min-h-[44px] flex-wrap items-center gap-1.5 rounded-xl bg-brand-soft/50 px-3 py-2 ring-1 ring-brand/15">
            {result.approvers.map((a, i) => (
              <span key={a} className="inline-flex items-center gap-1.5">
                <span className={cn('rounded-full px-2 py-0.5 text-[11px] font-bold', i === 0 ? 'bg-white text-brand-ink' : 'bg-lavender-soft text-lavender')}>{a}</span>
                {i < result.approvers.length - 1 ? <ArrowRight className="size-3.5 text-brand-ink" /> : null}
              </span>
            ))}
...
function RuleEditor({ rule, isNew, onCancel, onSave }: { rule: ApprovalRule; isNew: boolean; onCancel: () => void; onSave: (r: ApprovalRule) => void }) {
  const [draft, setDraft] = useState<ApprovalRule>(rule);
  const set = <K extends keyof ApprovalRule>(k: K, v: ApprovalRule[K]) => setDraft((d) => ({ ...d, [k]: v }));
  const toggleApprover = (role: ApproverRole) => {
    const has = draft.approvers.includes(role);
    const next = has ? draft.approvers.filter((a) => a !== role) : [...draft.approvers, role];
    // keep canonical chain order
    set('approvers', APPROVER_ROLES.filter((r) => next.includes(r)));
  };
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content aria-describedby={undefined} className="fixed right-0 top-0 z-[95] flex h-dvh w-[min(460px,94vw)] flex-col border-l border-glass-border-strong bg-glass-strong shadow-glass-lg backdrop-blur-glass-lg focus:outline-none">
          ...
          <div className="scrollbar-thin flex-1 space-y-4 overflow-y-auto p-5">
            <Field label="Rule name"><input value={draft.label} onChange={(e) => set('label', e.target.value)} className={inputCls} /></Field>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Category"><select value={draft.category} onChange={(e) => set('category', e.target.value as RuleCategory)} className={inputCls}>{RULE_CATEGORIES.map((c) => <option key={c} value={c}>{c === 'all' ? 'All types' : cap(c)}</option>)}</select></Field>
              <Field label="Entity"><select value={draft.scope} onChange={(e) => set('scope', e.target.value as EntityScope)} className={inputCls}><option value="all">All entities</option>{seedEntities.map((e) => <option key={e.id} value={e.id}>{e.name}</option>)}</select></Field>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Min amount (USD)"><input value={String(draft.minAmount)} onChange={(e) => set('minAmount', parseFloat(e.target.value.replace(/[^0-9.]/g, '')) || 0)} inputMode="decimal" className={inputCls} /></Field>
              <Field label="Max amount (blank = ∞)"><input value={draft.maxAmount === null ? '' : String(draft.maxAmount)} onChange={(e) => { const v = e.target.value.replace(/[^0-9.]/g, ''); set('maxAmount', v === '' ? null : parseFloat(v)); }} inputMode="decimal" className={inputCls} /></Field>
            </div>
            <div>
...
const shortRole = (r: ApproverRole) => (r === 'Finance Lead' ? 'Lead' : r === 'Organization Owner' ? 'Owner' : 'Board');
