...
export function SettingsLayout() {
  ...
  return (
    <div className="flex h-full flex-col">
      <PageHeader title="Settings" subtitle="Manage your organization, people, policies, integrations and billing." />
      <div className="flex min-h-0 flex-1 gap-5 px-8 pb-6">
        <GlassSurface tone="strong" className="flex w-[220px] shrink-0 flex-col gap-1 p-3">
          {TABS.map((t) => {
            const active = path === t.to || (path === '/settings' && t.to === '/settings/org');
            return (
              <Link key={t.to} to={t.to} className={cn('flex items-center gap-3 rounded-xl px-3 py-2.5 text-[13px] font-semibold transition-colors', active ? 'bg-white text-ink shadow-glass-soft' : 'text-ink-soft hover:bg-white/55 hover:text-ink')}>
