...
// "Link your work email" — connect flow shown until the user connects a mailbox.
export function ConnectMail() {
  const { email: userEmail } = useCurrentMailUser();
  ...
  const submit = () => {
    if (!email.trim()) { toast({ tone: 'warning', title: 'Enter your email' }); return; }
    connect(userEmail, email, provider);
          ...
          <div className="flex w-full flex-col gap-2 text-left">
            ...
            <div className="grid grid-cols-1 gap-2">
              {MAIL_PROVIDERS.map((p) => (
                <button key={p.id} type="button" onClick={() => setProvider(p.id)} className={cn('flex items-center justify-between rounded-2xl px-4 py-3 text-[13px] font-semibold ring-1 transition-colors', provider === p.id ? 'bg-white text-ink ring-brand/30' : 'bg-white/55 text-ink-soft ring-white/60 hover:bg-white')}>
                  {p.name}
                  {provider === p.id ? <Check className="size-4 text-brand" /> : null}
                </button>
              ))}
            </div>
            <label className="mt-2 text-[11px] font-bold uppercase tracking-wider text-ink-muted">Email address</label>
            <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@company.com" className="h-11 rounded-2xl bg-white/70 px-4 text-[14px] text-ink placeholder:text-ink-muted ring-1 ring-white/70 focus:outline-none focus:ring-2 focus:ring-brand/30" />
          </div>

          <button type="button" onClick={submit} className="mt-2 inline-flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-br from-brand to-brand-ink text-[14px] font-bold text-white shadow-glass-soft hover:brightness-110">
            Connect mailbox
          </button>
