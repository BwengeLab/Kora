...
import { seedRelationships, type RelationshipRowSeed } from '../../seed/orgOwnerHome';
...
export function ExternalRelationshipsCard() {
  return (
        ...
        {seedRelationships.map((r) => {
          ...
          return (
            ...
            >
              ...
              <span className="w-12 text-right text-[13px] font-bold text-ink tabular">{r.count}</span>
              <span
                className={cn(
                  'inline-flex w-32 items-center justify-end gap-1 text-[11px] font-semibold',
                  TONE_MAP[r.trendTone],
                )}
              >
                {r.trendTone === 'success' ? <ArrowUpRight className="size-3" /> : null}
                <span>{r.trendText}</span>
              </span>
            </li>
          );
        })}
