...
import { ArrowDownLeft, ArrowUpRight, CalendarClock, FileText, Mail, Phone, Search, Sparkles, X } from 'lucide-react';
...
// Org Owner "Relationships" — a working directory of every external party. Search,
// filter, then open a party to see their full history, balance, contacts and act
// (email, view statement, log a call). Orient → Work → Helpers.
export function RelationshipsPage() {
  ...
  return (
    <div className="flex h-full flex-col">
      <PageHeader title="Relationships" subtitle="Every external party — money in/out, open invoices, risk and contracts. Open one to see the full history and act." right={<DateRangePill label="May 2025" />} />
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 gap-5 @5xl:grid-cols-[1fr_300px]">
          ...
          <div className="flex flex-col gap-4">
            <GlassSurface tone="strong" className="flex flex-col gap-2.5 p-4">
              <header className="flex items-center gap-1.5"><CalendarClock className="size-3.5 text-warning" /><h4 className="text-[12px] font-bold text-ink">Upcoming renewals</h4></header>
              {seedRenewals.map((r) => (
                <div key={r.id} className="rounded-xl bg-white/55 p-2.5 text-[11.5px] ring-1 ring-white/60">
      ...
      <PartyDrawer party={selected} onClose={() => setSelected(null)} />
    </div>
  );
}

function PartyDrawer({ party: p, onClose }: { party: Party | null; onClose: () => void }) {
            ...
            <>
              ...
              <div className="scrollbar-thin flex-1 space-y-4 overflow-y-auto p-5">
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <Mini label="Contracts" value={String(p.contracts)} />
              ...
              <footer className="flex items-center gap-2 border-t border-white/55 p-4">
                <button type="button" onClick={() => openDoc({ name: `${p.name} — statement.pdf`, kind: 'statement', sizeText: '—', context: p.name })} className="inline-flex h-11 items-center justify-center gap-2 rounded-2xl bg-white/70 px-4 text-[13px] font-bold text-ink ring-1 ring-white/70 hover:bg-white"><FileText className="size-4" /> Statement</button>
                <button type="button" onClick={() => toast({ tone: 'success', title: 'Email drafted', body: `Opening a message to ${p.contact} <${p.email}>.` })} className="inline-flex h-11 flex-1 items-center justify-center gap-2 rounded-2xl bg-gradient-to-br from-brand to-brand-ink text-[13px] font-bold text-white shadow-glass-soft hover:brightness-110"><Mail className="size-4" /> Email contact</button>
              </footer>
            </>
          ) : null}
        </Dialog.Content>
