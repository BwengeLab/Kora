...
const SETTLE_LIMIT = 100000_00n; // $100,000 in minor units → above this needs dual approval
    ...
    <GlassSurface tone="strong" className="flex h-full min-h-0 flex-col">
      ...
      <div className="scrollbar-thin mt-5 flex-1 space-y-5 overflow-y-auto px-7 pb-6">
        ...
        <section className="flex flex-wrap gap-3">
          ...
          {claim.paymentReconciled !== null ? (
            <Status ok={claim.paymentReconciled} okText="Payment reconciled to bank" badText="Payment reconciling…" />
          ) : null}
        </section>
      ...
      <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-white/55 bg-white/45 px-7 py-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {claim.fraudScore >= 70 ? (
            <button type="button" onClick={() => { referSIU(claim.id); toast({ tone: 'danger', title: 'Referred to SIU', body: `${claim.id} flagged for fraud investigation.` }); }} className="inline-flex h-11 items-center gap-2 rounded-2xl bg-white px-4 text-[13.5px] font-bold text-danger ring-1 ring-danger/25 hover:bg-danger-soft">
              <ShieldAlert className="size-4" /> Refer to SIU
            </button>
          ) : null}
          <button type="button" onClick={() => toast({ tone: 'info', title: 'Documents requested', body: 'Request sent to the claimant.' })} className="inline-flex h-11 items-center gap-2 rounded-2xl bg-white/65 px-4 text-[13.5px] font-bold text-ink-soft ring-1 ring-white/70 hover:bg-white hover:text-ink">
            <Forward className="size-4" /> Request docs
          </button>
          {p ? (
            <button type="button" onClick={doAdvance} className="inline-flex h-11 items-center gap-2.5 rounded-2xl bg-gradient-to-br from-brand to-brand-ink px-5 text-[14px] font-bold text-white shadow-[0_8px_22px_rgba(67,97,238,0.45)] hover:brightness-110">
