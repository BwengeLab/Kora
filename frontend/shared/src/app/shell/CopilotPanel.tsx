...
export function CopilotPanel() {
  ...
  const send = (text: string) => {
    ...
    const koraMsg: Msg = { id: `k-${Date.now()}`, from: 'kora', text: answer(text) };
    ...
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Portal>
        ...
        <Dialog.Content
          ...
          className="fixed right-0 top-0 z-[95] flex h-dvh w-[min(420px,94vw)] flex-col border-l border-glass-border-strong bg-glass-strong shadow-glass-lg backdrop-blur-glass-lg focus:outline-none"
        >
          <header className="flex items-center gap-3 border-b border-white/55 px-5 py-4">
            <span className="grid size-10 place-items-center rounded-2xl bg-gradient-to-br from-ai to-brand text-white shadow-[0_4px_12px_rgba(139,92,246,0.4)]">
              <Sparkles className="size-5" />
            </span>
            <div className="flex-1">
              <Dialog.Title className="font-display text-[15px] font-bold text-ink">Kora AI</Dialog.Title>
              <p className="text-[11.5px] text-ink-muted">Your finance copilot · grounded in your data</p>
            </div>
            <Dialog.Close className="grid size-8 place-items-center rounded-lg text-ink-muted hover:bg-white/70 hover:text-ink">
              <X className="size-4" />
            </Dialog.Close>
          </header>
          ...
          <div className="scrollbar-thin flex-1 space-y-3 overflow-y-auto p-5">
            {messages.length === 0 ? (
              <div className="rounded-2xl bg-white/55 p-4 ring-1 ring-white/60">
                <p className="text-[13.5px] font-semibold text-ink">Hi {firstName} 👋</p>
                <p className="mt-1 text-[12.5px] text-ink-soft">
                  I can see your live {roleName} workspace. Ask me anything, or try one of these:
                </p>
              </div>
            ) : null}
            {messages.map((m) => (
                ...
                <div
                  className={cn(
                    ...
                    m.from === 'user'
                      ...
                      : 'bg-white/70 text-ink ring-1 ring-white/60',
                  )}
                >
                  {m.text}
                </div>
          ...
          <div className="flex flex-wrap gap-1.5 px-5 pb-2">
            {prompts.map((p) => (
              <button
                ...
                type="button"
                onClick={() => send(p)}
                className="rounded-full bg-white/60 px-2.5 py-1 text-[11px] font-semibold text-ink-soft ring-1 ring-white/60 hover:bg-white hover:text-ink"
              >
                {p}
              </button>
            ))}
          ...
          <form
            ...
            className="flex items-center gap-2 border-t border-white/55 p-4"
          >
            <input
              value={input}
              onChange={(e) => setInput(e.target.value)}
              placeholder="Ask Kora…"
              className="h-11 flex-1 rounded-2xl bg-white/70 px-4 text-[13px] text-ink placeholder:text-ink-muted ring-1 ring-white/60 focus:outline-none focus:ring-2 focus:ring-brand/30"
            />
            <button
              type="submit"
              aria-label="Send"
              className="grid size-11 place-items-center rounded-2xl bg-gradient-to-br from-brand to-brand-ink text-white shadow-glass-soft hover:brightness-110"
            >
...
function makeAnswerer(
  approvals: import('../../seed/approvals').ApprovalItem[],
  recons: import('../../seed/reconciliation').Reconciliation[],
) {
